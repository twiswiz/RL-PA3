...
class SensorTransmissionEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.MultiDiscrete(
            np.array([
                ...
                self.B + 1,
                self.num_pollution_states,
                self.num_pollution_states], dtype=np.int64))

        #we define all 3 actions globally.
    ...
    def reset(self, seed=None, options=None):
        ...
        # Fourth state: At the beginning of an episode, no previous unreported pollution history exists.
        # So max since last successful transmission is initialized to current theta.
        max_idx = theta_idx

        self.state = np.array([theta_idx, battery, estimate_idx, max_idx],dtype=np.int64)
    ...
    def step(self, action):
        ...
        # If battery is insufficient, transmission actions are not actually possible so no transmission.
        ...
        solar_energy = self.np_random.choice(np.arange(self.Delta + 1),p=self.alpha)

        # Update battery
        next_battery = min(self.B, battery_after_action + solar_energy)

        # Sample next pollution theta_{t+1}
        next_theta_idx = self.np_random.choice(np.arange(self.num_pollution_states),p=self.P[theta_idx])
        ...
        self.state = np.array([next_theta_idx, next_battery, new_estimate_idx, next_max_idx],dtype=np.int64)
